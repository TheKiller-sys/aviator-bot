...
WEBHOOK_URL = os.environ.get("WEBHOOK_URL")  # Nueva variable para la URL del webhook
PORT = int(os.environ.get("PORT", 5000))    # Puerto para el servidor web
...
# --- Webhook Setup --- (NUEVA SECCIÓN AL FINAL)
app = Flask(__name__)
bot = telebot.TeleBot(TOKEN)

@app.route('/')
def index():
    return "ServiceJ Bot está funcionando correctamente! 🚀"
...
if __name__ == '__main__':
    ...
    if WEBHOOK_URL:
        ...
        log_message = f"Webhook configurado en {WEBHOOK_URL}/webhook"
        logging.info(log_message)
        send_log_to_group(log_message)
    else:
        log_message = "⚠️ WEBHOOK_URL no configurada - Usando polling"
        logging.warning(log_message)
        send_log_to_group(log_message)
