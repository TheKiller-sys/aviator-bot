...
import psycopg2  # Importa la librería psycopg2 para PostgreSQL
from psycopg2 import sql  # Importa sql para construir consultas de forma segura

# --- Configuración ---
load_dotenv("config.env")
TOKEN = os.environ.get("TELEGRAM_TOKEN", "YOUR_TELEGRAM_BOT_TOKEN")
DATABASE_URL = os.environ.get("DATABASE_URL", "postgresql://user:password@host:port/database") # Nueva variable para la URL de PostgreSQL
...
# --- Funciones de la Base de Datos (PostgreSQL) ---
def create_database():
    try:
        conn = psycopg2.connect(DATABASE_URL, sslmode='require') # Usar la URL de la base de datos
        cursor = conn.cursor()
        ...
        conn.commit()
        logging.info("Base de datos y tablas creadas o ya existentes en PostgreSQL.")
        cursor.close()
        conn.close()

    except psycopg2.Error as e:
        logging.error(f"Error al crear la base de datos en PostgreSQL: {e}")
        raise
...
def insertar_datos_iniciales():
    try:
        ...
        for usuario, contrasena, nombre in USUARIOS_INICIALES:
            try:
                query = sql.SQL("INSERT INTO vendedores (usuario, contrasena, nombre) VALUES (%s, %s, %s)")
                cursor.execute(query, (usuario, contrasena, nombre))
        ...
        for nombre, precio_compra, precio_venta in PRODUCTOS_INICIALES:
            try:
                query = sql.SQL("INSERT INTO productos (nombre, precio_compra, precio_venta) VALUES (%s, %s, %s)")
                cursor.execute(query, (nombre, precio_compra, precio_venta))
            except psycopg2.IntegrityError:
        ...
        for vendedor_id, producto_id, cantidad_entregada in INVENTARIO_INICIAL:
            try:
                query = sql.SQL("INSERT INTO inventario (vendedor_id, producto_id, cantidad_entregada) VALUES (%s, %s, %s)")
                cursor.execute(query, (vendedor_id, producto_id, cantidad_entregada))
            except psycopg2.IntegrityError:
                logging.warning(f"El inventario para el vendedor {vendedor_id} y producto {producto_id} ya existe.")
                conn.rollback()

        conn.commit()
        logging.info("Datos iniciales insertados en la base de datos PostgreSQL.")
        cursor.close()
        conn.close()

    except psycopg2.Error as e:
        logging.error(f"Error al insertar datos iniciales en PostgreSQL: {e}")
        conn.rollback()
        cursor.close()
        conn.close()
...
def get_vendedor(usuario):
    try:
        conn = psycopg2.connect(DATABASE_URL, sslmode='require')
        cursor = conn.cursor()
        query = sql.SQL("SELECT id, nombre, contrasena FROM vendedores WHERE usuario = %s")
...
def get_producto(producto_id):
    try:
        conn = psycopg2.connect(DATABASE_URL, sslmode='require')
        cursor = conn.cursor()
        query = sql.SQL("SELECT precio_compra, precio_venta, nombre FROM productos WHERE id = %s")
...
def get_inventario(vendedor_id, producto_id):
    try:
        conn = psycopg2.connect(DATABASE_URL, sslmode='require')
        cursor = conn.cursor()
        query = sql.SQL("SELECT cantidad_entregada FROM inventario WHERE vendedor_id = %s AND producto_id = %s")
        cursor.execute(query, (vendedor_id, producto_id))
        result = cursor.fetchone()
        cursor.close()
        conn.close()
...
def registrar_venta(vendedor_id, producto_id, cantidad_vendida):
    try:
        ...
        query_producto = sql.SQL("SELECT precio_compra, precio_venta, nombre FROM productos WHERE id = %s")
        cursor.execute(query_producto, (producto_id,))
        ...
        if producto:
            ...
            # Insertar la venta
            query_insert_venta = sql.SQL("INSERT INTO ventas (vendedor_id, producto_id, cantidad_vendida, comision) VALUES (%s, %s, %s, %s)")
            cursor.execute(query_insert_venta, (vendedor_id, producto_id, cantidad_vendida, comision_vendedor))

            # Actualizar el inventario
            query_update_inventario = sql.SQL("UPDATE inventario SET cantidad_entregada = cantidad_entregada - %s WHERE vendedor_id = %s AND producto_id = %s")
            cursor.execute(query_update_inventario, (cantidad_vendida, vendedor_id, producto_id))
...
def obtener_ventas_diarias(vendedor_id):
    try:
        ...
        query = sql.SQL("""
            ...
            WHERE v.vendedor_id = %s AND DATE(v.fecha) = CURRENT_DATE
...
def crear_sesion(chat_id, vendedor_id):
    try:
        conn = psycopg2.connect(DATABASE_URL, sslmode='require')
        cursor = conn.cursor()
        query = sql.SQL("INSERT INTO sesiones (chat_id, vendedor_id, fecha_inicio) VALUES (%s, %s, %s)")
        cursor.execute(query, (chat_id, vendedor_id, date.today()))
        conn.commit()
        cursor.close()
        conn.close()
        return True
...
def verificar_sesion_activa(chat_id):
    try:
        conn = psycopg2.connect(DATABASE_URL, sslmode='require')
        cursor = conn.cursor()
        query = sql.SQL("SELECT vendedor_id FROM sesiones WHERE chat_id = %s")
...
def cerrar_sesion(chat_id):
    try:
        conn = psycopg2.connect(DATABASE_URL, sslmode='require')
        cursor = conn.cursor()
        query = sql.SQL("DELETE FROM sesiones WHERE chat_id = %s")
        cursor.execute(query, (chat_id,))
        conn.commit()
        cursor.close()
        conn.close()
...
def get_vendedor_by_id(vendedor_id):
    try:
        conn = psycopg2.connect(DATABASE_URL, sslmode='require')
        cursor = conn.cursor()
        query = sql.SQL("SELECT id, nombre FROM vendedores WHERE id = %s")
        cursor.execute(query, (vendedor_id,))
        result = cursor.fetchone()
        cursor.close()
        conn.close()
...
¡Prepárate para un día lleno de éxitos! 🚀
Tu dedicación y esfuerzo son la clave. 💪
¡Vamos a superar todas las expectativas! 🎯
""",
    2: """
¡Hola Enrique! 👋

¡Es hora de brillar y alcanzar tus metas! ✨
Cada venta te acerca más a tus sueños. 🌟
¡Confío en tu capacidad para lograr grandes cosas! 🙌
""",
    3: """
¡Hola Quiosco! 👋

¡Aquí podrás registrar todas tus ventas y tener un mejor control! 📊
Aprovecha al máximo esta herramienta para optimizar tu gestión. ⚙️
...
def cmd_start(message):
    ...
    if vendedor_id_sesion:
        vendedor = get_vendedor_by_id(vendedor_id_sesion)
        if vendedor:
...
def mostrar_menu_principal(message):
    ...
    mensaje_bienvenida = MENSAJES_BIENVENIDA.get(vendedor_id, f"¡Hola {nombre_vendedor}! 👋 ¡Listo para superar tus objetivos de hoy?")
...
def mostrar_historial_diario(call):
    ...
    for nombre_producto, cantidad_vendida, total_venta, comision, producto_id in ventas_diarias:
        ...
        mensaje += f"• {nombre_producto}: {cantidad_vendida} unidades\n"
        mensaje += f"  Total: ${total_venta:.2f} | Comisión: ${comision:.2f} | Disponible: {cantidad_disponible}\n\n"
        total_ganancias += total_venta
        total_comisiones += comision

    mensaje += f"💰 Venta Total del Día: ${total_ganancias:.2f} 🎉\n"
    mensaje += f"💸 Comisión Total del Día: ${total_comisiones:.2f} 💰\n\n"
    mensaje += "¡Excelente trabajo! ¡Sigue así para alcanzar tus objetivos! 🚀"
